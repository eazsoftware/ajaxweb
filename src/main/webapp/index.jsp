<%@page contentType="text/html" pageEncoding="UTF-8"%>
<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN"
   "http://www.w3.org/TR/html4/loose.dtd">

<html>
    <head>
        <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
        <title>Ajax Web</title>        
    </head>
    <body>
        <h1>Ajax Web</h1>
        <ul>
            <li><a href="simpleAjax/simpleAjax.jsp">Simple Ajax call</a></li>
            <li><a href="simpleAjax/w3school.jsp">W3School Ajax call</a></li>
            <li><a href="jQuery/jQueryAjax-load.jsp">jQuery Ajax Load()</a></li>
            <li><a href="jQuery/jQueryAjax-loadDOM.jsp">jQuery Ajax Load() DOM#</a></li>
            <li><a href="jQuery/jQueryAjax-callback.jsp">jQuery Ajax Function</a></li>          
            <li><a href="jQuery/jQueryAjax-noloadGoogle.jsp">jQuery Ajax - no load Google jQuery library</a></li> 
        </ul>
        
        <h1>jSON</h1>
        <ul>
            <li><a href="jSON/jSON-postermovie.jsp">A responsive movie poster grabber using jSON.</a></li>
            <li><a href="jSON/jSON-w3school-createsimplejSON.jsp">jSON - w3schools - create simple jSON object.</a></li>
            <li><a href="jSON/jSON-w3school-parseObjectFromString.jsp">jSON - w3schools - parse Object from String.</a></li>            
            <li><a href="jSON/jSON-twitter-usertimeline.jsp">jSON - twitter - user timeline tweets.</a></li>            
        </ul>        
    </body>
</html>
